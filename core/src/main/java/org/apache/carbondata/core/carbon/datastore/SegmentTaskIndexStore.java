...
public class SegmentTaskIndexStore<K, V>
  ...
  @Override public SegmentTaskIndexWrapper getIfPresent(
    ...
    SegmentTaskIndexWrapper segmentTaskIndexWrapper = (SegmentTaskIndexWrapper) lruCache.get(
        ...
        if (segmentTaskIndexWrapper == null) {
          ...
          synchronized (segmentLoderLockObject) {
            ...
            if (null == segmentTaskIndexWrapper) {
              ...
              Iterator<Map.Entry<String, List<TableBlockInfo>>> iterator =
              ...
              if (isAddedToLruCache) {
                while (iterator.hasNext()) {
                  Map.Entry<String, List<TableBlockInfo>> taskToBlockInfoList = iterator.next();
  ...
  private long calculateRequiredSize(Map<String, List<TableBlockInfo>> taskIdToTableBlockInfoMap,
      AbsoluteTableIdentifier absoluteTableIdentifier) {
    Iterator<Map.Entry<String, List<TableBlockInfo>>> iterator =
        taskIdToTableBlockInfoMap.entrySet().iterator();
    String taskId;
    ...
    while (iterator.hasNext()) {
      Map.Entry<String, List<TableBlockInfo>> taskToBlockInfoList = iterator.next();
      taskId = taskToBlockInfoList.getKey();
      driverBTreeSize += CarbonUtil.calculateDriverBTreeSize(taskId, taskToBlockInfoList.getValue(),
  ...
  private Map<String, List<TableBlockInfo>> mappedAndGetTaskIdToTableBlockInfo(
    ...
    Iterator<Map.Entry<String, List<TableBlockInfo>>> iterator =
        segmentToTableBlocksInfos.entrySet().iterator();
    while (iterator.hasNext()) {
      Map.Entry<String, List<TableBlockInfo>> next = iterator.next();
      List<TableBlockInfo> value = next.getValue();
      for (TableBlockInfo blockInfo : value) {
        String taskNo = CarbonTablePath.DataFileUtil.getTaskNo(blockInfo.getFilePath());
